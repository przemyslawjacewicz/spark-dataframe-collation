...
class CollationDataFrameJoinUsingColumnSpec extends AnyFlatSpec with SparkSessionProvider with Matchers {
  ...
  it should "fallback to dataframe join when joined using non-normalized column" in {
    val df    = Seq((1, "a"), (2, "b")).toDF()
    val right = Seq((1, "A")).toDF()

    val r = df.c.join(right, "_1")
  ...
  it should "join dataframes with normalization when joined using normalized column" in {
    val df    = Seq((1, "a"), (2, "b")).toDF()
    val right = Seq((1, "A")).toDF()

    val r = df.c.join(right, "_2")
